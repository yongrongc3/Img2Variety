...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline+pretrain+StMA_DenseNet_XXX.pdf")
...
for epoch in range(150):
    ...
    if epoch > 148:
        save_folder = '/models'
        save_path = os.path.join(save_folder, f'baseline+pretrain+StMA_DenseNet_XXX_{epoch}.pth')
...
csv_file = './results/baseline+pretrain+StMA_DenseNet_XXX_pred.csv'
...
plt.savefig('./results/baseline+pretrain+StMA_DenseNet_XXX_ConfusionMatrix.pdf', format='pdf')
