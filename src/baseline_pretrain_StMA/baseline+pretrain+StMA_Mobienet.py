...
# model = MobileNetV2(num_classes=224).to(device)
...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline+pretrain+StMA_Mobienet_XXX.pdf")
...
for epoch in range(150):
    ...
    if epoch > 140:
        save_folder = '/models'
        save_path = os.path.join(save_folder, f'baseline+pretrain+StMA_Mobienet_XXX_{epoch}.pth')
...
csv_file = './results/baseline+pretrain+StMA_Mobienet_XXX.csv'
...
plt.savefig('./results/baseline+pretrain+StMA_Mobienet_XXX_ConfusionMatrix.pdf', format='pdf')
