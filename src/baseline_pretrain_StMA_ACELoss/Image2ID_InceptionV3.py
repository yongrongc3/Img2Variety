...
criterion = WeightedLoss(alpha=1.0)
...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_Image2ID_InceptionV3_XXX.pdf")
losses = []
for epoch in range(1):
    ...
    # if epoch > 140:
    save_folder = './models'
    save_path = os.path.join(save_folder, f'Image2ID_InceptionV3_XXX_{epoch}.pth')
...
csv_file = './results/Image2ID_InceptionV3_XXX_pred.csv'
...
plt.savefig('./results/Image2ID_InceptionV3_XXX_ConfusionMatrix.pdf', format='pdf')
