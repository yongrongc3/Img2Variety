...
model = DenseNet(num_classes=93).to(device)
# 224 maize lines identification
# model = DenseNet(num_classes=224).to(device)
...
criterion = WeightedLoss(alpha=0.002)
...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline_pretrain_ACELoss_DenseNet_XXX.pdf")
...
for epoch in range(150):
    ...
    if epoch > 140:
        save_folder = './models'
        save_path = os.path.join(save_folder, f'baseline_pretrain_ACELoss_DenseNet_XXX_{epoch}.pth')
...
csv_file = './results/baseline_pretrain_ACELoss_DenseNet_XXX.csv'
...
plt.savefig('./results/baseline_pretrain_ACELoss_DenseNet_XXX_ConfusionMatrix.pdf', format='pdf')
