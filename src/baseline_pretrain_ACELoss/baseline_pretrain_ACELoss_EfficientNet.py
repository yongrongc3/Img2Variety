...
criterion = WeightedLoss(alpha=0.9)
...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline_pretrain_ACELoss_EfficientNet_XXX.pdf")
...
for epoch in range(150):
    ...
    if epoch > 140:
        save_folder = './models'
        save_path = os.path.join(save_folder, f'baseline_pretrain_ACELoss_EfficientNet_XXX_{epoch}.pth')
        ...
        print(f"Epoch:{epoch}\tTest Loss: {test_loss:.9f}\tAccuracy: {test_acc * 100:.4f}%\tPrecision: {precision* 100:.4f}%\t, Recall: {recall* 100:.4f}%\t, F1 Score: {f1* 100:.4f}%\t")
        csv_filename = './results/baseline_pretrain_ACELoss_EfficientNet_XXX.csv'
