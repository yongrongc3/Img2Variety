...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline_pretrain_ACELoss_ResNet_XXX.pdf")
losses = []
...
for epoch in range(150):
    ...
    if epoch > 140:
        save_folder = './models'
        save_path = os.path.join(save_folder, f'baseline_pretrain_ACELoss_ResNet_XXX_{epoch}.pth')
...
csv_file = './results/baseline_pretrain_ACELoss_ResNet_XXX_pred.csv'
...
plt.savefig('./results/baseline_pretrain_ACELoss_ResNet_XXX_ConfusionMatrix.pdf', format='pdf')
