...
model = VGG19(num_classes=93).to(device)
# 224 maize lines identification
# model = VGG19(num_classes=224).to(device)
...
criterion = WeightedLoss(alpha=0.2)
...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline_pretrain_ACELoss_VGG_XXX.pdf")
losses = []
...
for epoch in range(150):
    ...
    print('Epoch: [%d] loss: %.9f' % (epoch + 1, running_loss * 64 / len(train_dataset)))
    if epoch > 140:
        save_folder = './models'
        save_path = os.path.join(save_folder, f'baseline_pretrain_ACELoss_VGG_XXX_{epoch}.pth')
...
print("Test Loss: {:.9f}, Accuracy: {:.4f}%".format(test_loss, test_acc * 100))
...
csv_file = './results/baseline_pretrain_ACELoss_VGG_XXX.csv'
...
plt.savefig('./results/baseline_pretrain_ACELoss_VGG_XXX_ConfusionMatrixal.pdf', format='pdf')
