...
criterion = WeightedLoss(alpha=0.01)
for epoch in range(141, 150):
    model.load_state_dict(torch.load(f'./models/baseline_pretrain_ACELoss_DenseNet_XXX_{epoch}.pth'))
    print(f"epoch:{epoch}")
    model.eval()
    test_loss = 0.0
    test_acc = 0.0
...
csv_file = './results/test_baseline_pretrain_ACELoss_DenseNet_XXX_pred.csv'
