...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline+pretain_MobilenetV2_XXX.pdf")
...
for epoch in range(150):
    ...
    if epoch > 140:
        save_folder = '/models'
        save_path = os.path.join(save_folder, f'baseline+pretain_MobilenetV2_XXX_{epoch}.pth')
...
csv_file = './results/baseline+pretain_MobilenetV2_XXX_pred.csv'
...
plt.savefig('./results/baseline+pretain_MobilenetV2_XXX_ConfusionMatrix.pdf', format='pdf')
