...
model = InceptionNetV3(num_classes=93).to(device)
# 224 maize lines identification
# model = InceptionNetV3(num_classes=224).to(device)
# 2 rice subspecies classification
# model = InceptionNetV3(num_classes=2).to(device)
...
def Draw_loss(losses):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig("./results/loss_baseline+pretrain_InceptionV3_XXX.pdf")
...
for epoch in range(150):
    ...
    for i, (inputs, labels) in enumerate(train_loader, 0):
        ...
        # output logits
        if isinstance(outputs, tuple):  # InceptionV3 outputs a tuple (logits, aux_logits) if aux_logits=True
            outputs = outputs[0]
    ...
    if epoch > 140:
        save_folder = '/models'
        save_path = os.path.join(save_folder, f'baseline+pretrain_InceptionV3_XXX_{epoch}.pth')
...
csv_file = './results/baseline+pretrain_InceptionV3_XXX_pred.csv'
...
plt.savefig('./results/baseline+pretrain_InceptionV3_XXX_ConfusionMatrix.pdf', format='pdf')
